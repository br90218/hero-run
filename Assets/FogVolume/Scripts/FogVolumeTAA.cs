...
public class FogVolumeTAA : FogVolumePlaydeadTAA.EffectBase
{
    ...
    public Neighborhood neighborhood = Neighborhood.MinMax3x3;
    ...
    FogVolumeCamera _FogVolumeCamera;
    ...
    void Resolve(RenderTexture source, RenderTexture destination)
    {
        ...
        if (EnsureRenderTarget(ref reprojectionBuffer[eyeIndex, 0], bufferW, bufferH, _FogVolumeCamera.GetRTFormat(), FilterMode.Bilinear, antiAliasing: source.antiAliasing))
            Clear();
        if (EnsureRenderTarget(ref reprojectionBuffer[eyeIndex, 1], bufferW, bufferH, _FogVolumeCamera.GetRTFormat(), FilterMode.Bilinear, antiAliasing: source.antiAliasing))
            Clear();
    ...
    {
        RenderTexture internalDestination = RenderTexture.GetTemporary(source.width, source.height, 0, _FogVolumeCamera.GetRTFormat(), RenderTextureReadWrite.Default, source.antiAliasing);
        {
            Resolve(source, internalDestination);
